...
- 2026-06-25: Diagnosed production contact 400s: the live POST endpoint works with valid data, while short messages failed the server's 20-character validation. Added `minLength={20}` to the message textarea and clearer API validation copy. Removed the test lead created during verification. Verified `npm run build` passes.
- 2026-06-25: Added full contact-form validation handling: client-side field validation with inline error messages, invalid field styling, `noValidate` custom handling, server-side Zod field messages, and API responses that return field-specific validation errors. Verified `npm run build` passes.
